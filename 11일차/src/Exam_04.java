...
//추가되는 내용을 구현하기 위한 자식 클래스 생성
class Computer2 extends Computer{
	//name, company,price,get~~~,set~~~~는 가지고 시작
	
	private String as;//AS기간이라는 항목을 추가, 설정, 변경 가능

	public Computer2 (String name, String company, int price, String as){
		super(name, company, price); //부모의 생성자 호출
		this.setAs(as);
	}
...
public class Exam_04 {
	public static void main(String[] ar){
		...
		for(int i=0; i<com.length; ++i){
			...
			sc.nextLine();
			System.out.println("AS기간 : ");
			String as= sc.nextLine();
			com[i] = new Computer2(name, company, price,as);
		}
		
		for(int i=0; i<com.length; ++i){
			System.out.printf("이름 : %s, 제조사 : %s, 가격 : %d, AS : %s\n",
